...
import { Layout, User, Mail, Lock, Loader2, UserPlus } from 'lucide-react';
    ...
    <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl border border-neutral-200 shadow-2xl shadow-neutral-200/50 p-8 md:p-12">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-extrabold text-neutral-900 tracking-tight">Создать аккаунт</h1>
          <p className="text-neutral-500 mt-2">Присоединяйтесь к сообществу ScienceHub</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-neutral-700 ml-1">Полное имя</label>
            <div className="relative group">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-blue-500 transition-colors" size={20} />
              <input
                {...register('fullName')}
            ...
            {errors.fullName && <p className="text-red-500 text-xs mt-1 ml-1">{(errors.fullName as any).message}</p>}
          </div>
          ...
          >
            {isSubmitting ? <Loader2 className="animate-spin" /> : 'Зарегистрироваться'}
          </button>
        </form>
